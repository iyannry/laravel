    ...
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                ...
                @else
                  <img src="https://source.unsplash.com/700x1000?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}">
                @endif
                <div class="card-body">
                  <h5 class="card-title">{{ $post->title }}</h5>
                  <p>
                    <small class="text-muted">
                    By. <a href="/posts?author={{ $post->author->username }}" class="text-decoration-none text-secondary">{{ $post->author->name }}</a> {{ $post->created_at->diffForHumans() }}
                    </small>
                  </p> 
